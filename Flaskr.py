...
def show_post(post_title):
	...
	text = cur.fetchall()
	cur = db.execute('select name, comment from comments where id = (?) order by name desc',[title])
	entries = cur.fetchall()
	return render_template('comment_entries.html',title =title,text=text, entries=entries)	

@app.route('/comment_entry', methods=['GET','POST'])
def comment_entry(title):
	db = get_db()
	name = request.form['name']
	text = request.form['text']
	db.execute('insert into comments(id,name,comment) values (?,?,?)',[title,name,text])
	db.commit()
	cur = db.execute('select name,comment from comments where id=(?) order by name desc',[title])
	entries = cur.fetchall()
